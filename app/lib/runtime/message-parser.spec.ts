...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out foil artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <foi', 'Foo bar '],
      ['Foo bar <foil', 'Foo bar '],
      ['Foo bar <foila', 'Foo bar <foila'],
      ['Foo bar <foilA', 'Foo bar '],
      ['Foo bar <foilArtifacs></foilArtifact>', 'Foo bar <foilArtifacs></foilArtifact>'],
      ['Before <oltArtfiact>foo</foilArtifact> After', 'Before <oltArtfiact>foo</foilArtifact> After'],
      ['Before <foilArtifactt>foo</foilArtifact> After', 'Before <foilArtifactt>foo</foilArtifact> After'],
    ])('should correctly parse chunks and strip out foil artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <foilArtifact title="Some title" id="artifact_1">foo bar</foilArtifact> Some more text',
        {
      ...
      [
        ['Some text before <boltArti', 'fact', ' title="Some title" id="artifact_1">foo</foilArtifact> Some more text'],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
      ...
      [
        [
          ...
          ' ',
          '>',
          'foo</foilArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <foilArtifact title="Some title" id="artifact_1">foo</foilArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <foilArtifact title="Some title" id="artifact_1"><foilAction type="shell">npm install</foilAction></foilArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <foilArtifact title="Some title" id="artifact_1"><foilAction type="shell">npm install</foilAction><foilAction type="file" filePath="index.js">some content</foilAction></foilArtifact> After',
        {
          output: 'Before  After',
